  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container) {
        ...
        // Ensure the canvas doesn't intercept pointer events so page UI stays clickable
        try {
    ...
    class MouseClass {
      ...
      init(container) {
        this.container = container;
        // Make the visual canvas not block clicks — let clicks pass through
        // so UI elements (buttons) remain clickable. We listen to document
        // events for mouse/touch and map them into the container coordinates.
        try {
          this.container.style.pointerEvents = "none";
        } catch (e) {
          /* ignore (server/ssr guard) */
        }
      ...
      setCoords(x, y) {
        ...
        // mark that the pointer is inside the container bounds
        this.isHoverInside = true;
        this.mouseMoved = true;
        this.timer = setTimeout(() => {
          this.mouseMoved = false;
          // debounce clearing hover state shortly after movement stops
          this.isHoverInside = false;
        }, 100);
      }
      setNormalized(nx, ny) {
